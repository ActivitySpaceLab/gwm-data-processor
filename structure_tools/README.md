...
- **`calculate_monthly_participation.py`** – Aggregates initial + biweekly submissions per participant per calendar month (YYYY-MM columns)
...
4. Summarise monthly participation counts:
   ```bash
   python calculate_monthly_participation.py --input ../data/structured/latest --output ../data/structured/latest
   ```

5. Produce per-participant HTML reports:
   ```bash
   python generate_participant_reports.py --input ../data/structured/latest --output ../data/reports/latest
   ```

6. Analyze encryption performance:
   ```bash
   python analyze_encryption_limits.py
   ```

7. Analyze location data:
   ```bash
