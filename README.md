...
- `structure_tools/` – Data structuring (`generate_survey_csvs.py`), biweekly compliance analysis (`calculate_biweekly_periods.py`), monthly participation summary (`calculate_monthly_participation.py`), reporting (`generate_participant_reports.py`), and analytics helpers.
- `data/`
...
# 5. Summarise monthly participation counts
python structure_tools/calculate_monthly_participation.py \
  --input ./data/structured --output ./data/structured

# 6. Generate participant HTML reports (after structuring completes)
...
- `data/structured/monthly_participation_summary.csv` & `.md` (calendar month counts)
- `data/structured/monthly_participation_summary_latest.csv` (rolling monthly snapshot)
